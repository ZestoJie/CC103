...
public class DashboardController implements TimerService.TimerListener {
    ...
    private void loadTasks() throws Exception {
        ...
        String sql = "SELECT t.id, t.username, t.task_name, t.task_date, t.status, t.class_id, COALESCE(c.class_name, '') as class_name, t.is_personal "
                   ...
                   + "LEFT JOIN user_classes uc ON t.class_id = uc.class_id AND uc.user_id = ? "
    ...
    private void updateDailyGoalProgress() {
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            if (dailyGoalTip != null) {
                dailyGoalTip.setText(total == 0 ? "Enjoy your free time!" : (progress == 1.0 ? "Daily goal reached! ≡ƒÄë" : "Keep going!"));
            }
        } catch (Exception e) {
                ...
                leaderboardPreview.setCellFactory(param -> new ListCell<UserRank>() {
                    ...
                    protected void updateItem(UserRank item, boolean empty) {
                        ...
                        if (empty || item == null) {
                            ...
                            int rank = getIndex() + 1;
                            String rankIcon = (rank == 1) ? "≡ƒÑç " : (rank == 2) ? "≡ƒÑê " : (rank == 3) ? "≡ƒÑë " : rank + ". ";
